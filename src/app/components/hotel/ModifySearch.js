...
export default function ModifySearch(props) {
  ...
  const srchHtl = () => {
    ...
    if(allowMe){
      ...
      let paxInfoArr = []
      let indxCount = 0
      for (var k = 0; k < parseInt(rmCountArr.length); k++) {
        indxCount = indxCount + 1
        paxInfoArr.push({
          idAdt: `adt${k}`,
          idChd: `chd${k}`,
          idDelBtn: `delRoom${k}`,
          adtVal: qry['adults' + indxCount],
          chdVal: qry['children' + indxCount],
          chdAgesArr: [{
            idchdAges: 'chdAges0',
            chdAgeVal: qry['ca' + indxCount].replace(/,/g, '')[0],
            disabled: qry['ca' + indxCount][0] > 0 ? false : true
          },
          {
            idchdAges: 'chdAges1',
            chdAgeVal: qry['ca' + indxCount].replace(/,/g, '')[1],
            disabled: qry['ca' + indxCount][2] > 0 ? false : true
          },
          {
            idchdAges: 'chdAges2',
            chdAgeVal: qry['ca' + indxCount].replace(/,/g, '')[2],
            disabled: qry['ca' + indxCount][4] > 0 ? false : true
          },
          {
            idchdAges: 'chdAges3',
            chdAgeVal: qry['ca' + indxCount].replace(/,/g, '')[3],
            disabled: qry['ca' + indxCount][6] > 0 ? false : true
          }]
        })
      }
      qry.paxInfoArr = paxInfoArr
