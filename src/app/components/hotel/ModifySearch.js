...
export default function ModifySearch({Type, HtlReq, filterOpen}) {
  ...
  const [options, setOptions] = useState(HtlReq ? [{
    ...
    destinationCode: HtlReq.SearchParameter.DestinationCode,
    predictiveText:  HtlReq.SearchParameter.PredictiveText
  }]
  :
  [{
  ...
  const handleSearch = async (query) => {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_ROOT_API}/staticdata/DestinationsPrediction`, {
      method: 'POST',
      headers: {'Content-Type': 'application/json', 'domain': 'localhost:5001', 'correlation-id': userInfo?.correlationId},
      body: JSON.stringify({
      "text": query,
  ...
  const srchHtl = () => {
    let qry = {
      ...
      predictiveText: selectedDestination.length ? selectedDestination[0].predictiveText : HtlReq.SearchParameter.predictiveText,
      area: '',
      hotelId: '',
      hotelName: '',
      SearchTag: '',
      dest_code: selectedDestination.length ? selectedDestination[0].destinationCode : HtlReq.SearchParameter.DestinationCode,
    ...
    if(Type === 'result'){
      ...
      const params = new URLSearchParams(searchParams)
      //router.push(`/pages/hotelListing?${JSON.stringify(qry)}`);
      //const params = new URLSearchParams(searchParams)
      params.set('qry',JSON.stringify(qry))
      router.push(`/pages/hotelListing?${params}`);

      // params = qry;
      // const newParams = params.toString();
      // router.push(`/pages/hotelListing?${newParams}`);
      //router.push('/pages/hotelListing' + '?' + createQueryString('sort', JSON.stringify(qry)))
      // pathname: '/pages/hotelListing',
      //     query: {
      //       search: '333',
      //       march:'sds'
      //     }
      ...
      //router.push('/pages/hotelListing' + '?' + createQueryString('sort', JSON.stringify(qry)))
      // const str = JSON.stringify(qry);
      // console.log("str", str)
      //router.push(`/pages/hotelListing?${qry}`)

      // router.push({
      //   pathname: '/pages/hotelListing/[pid]',
      //   query: { pid: str },
      // })
      ...
      //router.push("/pages/hotelListing",{name:'riyaj'})
      // const str = 123;
      // router.push({
      //   pathname: 'pages/hotelListing',
      //   query: {
      //     myData: JSON.stringify({qry:'123'})
      //    }
      // });
  ...
  <div className="searchPanel">  
    ...
    <div className="searchBox">
      <div className="container">
        ...
        }} className="btn btn-warning px-4 py-2 fw-semibold">Search</Link> */}
              </div>
            </div>
          </div>
        </div>
