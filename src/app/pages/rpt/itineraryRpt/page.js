...
export default function ItineraryRpt() {
  ...
  const doInvoiceLoad = async() => {
    ...
    if(resRpt?.errorInfo ===null) {
      const resRptSort =  resRpt?.reportDetails?.sort((a, b) => dateFormater(a.bookedFrom) - dateFormater(b.bookedFrom))
      resRpt.reportDetails = resRptSort;
      let day = 1;
      if(resRpt.reportDetails){
        resRpt.reportDetails.map((v, i) => {
          if(i !== 0){
            if(dateFormater(resRpt.reportDetails[i-1].bookedFrom) !== dateFormater(v.bookedFrom)){
              day = day + 1
            }
          }
          v.day = day
